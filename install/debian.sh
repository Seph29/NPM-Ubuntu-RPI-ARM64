...
_distro_release=$(grep -Po 'VERSION="[0-9]+ \(\K[^)]+' /etc/os-release)
_distro_release=$(wget $WGETOPT "http://openresty.org/package/arm64/debian/dists/" -O - | grep -o "$_distro_release" | head -n1 || true)
echo "deb [trusted=yes] http://openresty.org/package/arm64/debian ${_distro_release:-buster} main" | tee /etc/apt/sources.list.d/openresty.list
# Install nodejs
